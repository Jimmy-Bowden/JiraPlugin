document.addEventListener(
  ...
  function() {
    ...
    dropdownTemplates.addEventListener("change", function() {
      ...
      if (template != null) {
        document.getElementById("no-template").style.display = "none";
        document.getElementById(template).style.display = "inline-block";
      } else {
        document.getElementById("no-template").style.display = "inline-block";
      }
    });
    ...
    function getCorrectUrl(tablink) {
      ...
      var urlMatch = urlTemp.match(
        /((https?|ftp|smtp):\/\/)?(www.)?([a-z0-9]+).atlassian/
      );
